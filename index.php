...
<body class="cd-body">
	...
	<nav>
		<ul class="cd-primary-nav">
			...
			<li class="cd-label" onclick="document.getElementById('id01').style.display='block'" style="cursor: pointer;">Log In <a href="login.php"></a></li>
						...
						<form name = "index" action = "search.php" method = "GET">
							...
							<select name="food" class="food-bar" required>
								...
								<?php
									...
									$statement = "select * from food order by CATEGORY";
									$s = oci_parse($c, $statement);
									oci_execute($s);
									$i='hello';
									while(($row = oci_fetch_assoc($s)) != false) 
